...
numOfTimes = int(input("How many times do you want to play "))

playerScore = 0
computerScore = 0
ties = 0

for i in range(1, numOfTimes+1):
    get_player_move()
    get_computer_move()
    if determine_winner(get_player_move(), get_computer_move()) == "player":
        playerScore += 1
    elif determine_winner(get_player_move(), get_computer_move()) == "computer":
        computerScore += 1
    else:
        ties += 1
    print_scoreboard(playerScore, computerScore, ties)
